 ...
 * high-resolution img to devices with retina displays.
 */
(function() {
			...
			jQuery.getJSON(api + opts.username + limitcount + opts.limit).done(function(data) 
			{
				...
				if ( opts.rotate === true ) {
					...
					function rotateTweets() {
					   listItem.eq(current++).fadeOut(500, function(){
							current = (current === listLength) ? 0 : current;
							listItem.eq(current).fadeIn(500);
					   });
					}
...
jQuery.fn.mainMenu = function(options){
	var settings = {
		 marker	 		:true     			// marker that indicates a submenu
...
$(".panel a").click(function(e){
	e.preventDefault();
		...
		$(".mainMenu").removeAttr("class").addClass("mainMenu").addClass(style);
	});
$().mainMenu();
